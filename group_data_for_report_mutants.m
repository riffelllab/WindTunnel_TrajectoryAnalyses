...
function output= group_data_for_report_mutants (fileNameList, typeGenderList, trajSmry, testedColorList, countInVolList, baseCueIndexList, piList, baseColor)
    %If we have data ~=0 in the trajSmry parameter, prerare the date for storage
    if nnz(trajSmry)     
        header= {'date', 'type', 'gender', 'numOfTrajectoriesAIR', 'avgTimeFlightAIR', 'maxTimeFlightAIR', 'numOfTrajectoriesCO2', 'avgTimeFlightCO2', 'maxTimeFlightCO2', 'numOfTrajectoriesPostCO2', 'avgTimeFlightPostCO2', 'maxTimeFlightPostCO2'};
        auxTable= horzcat(fileNameList, typeGenderList(:,1), typeGenderList(:,2), num2cell(trajSmry));
