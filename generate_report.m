...
%   - fileName: Name to use for the summary report file
...
function generate_report(outPath, fileName, expSheet, fileNameList, trajSmry, countInVolList, baseColorIndexList, testedColorList, piList)
     ...
     if isfile(strcat(outputFile))
        ...
        if any(strcmp(sheetList, expSheet))
            ...
            if cellfun(@isempty,newEntries)
                ...
                expData=[fileNameList(indexes1,:), num2cell(trajSmry(indexes1,:)),testedColorList(indexes1), num2cell(countInVolList(indexes1,:)), num2cell(baseColorIndexList(indexes1)), num2cell(piList(indexes1))];
                writetable([dataFromExcel; expData], outputFile, 'sheet',expSheet);
            end
            clear dataFromExcel
        else
            %If there is no sheet named as expSheet
            expData= group_data_for_report(fileNameList, trajSmry, testedColorList, countInVolList, baseColorIndexList, piList, baseColor);
            writecell(expData, outputFile, 'sheet',expSheet, 'Range', 'A1');
        end  
     else
        output= group_data_for_report (fileNameList, trajSmry, testedColorList, countInVolList, baseColorIndexList, piList, baseColor);
