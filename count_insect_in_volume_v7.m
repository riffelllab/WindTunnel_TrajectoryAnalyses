...
%   - dataEntry: dataset for a given experiment (matrix of matrices) with
%   data related to the odor used
...
function countsPerPosition= count_insect_in_volume_v7(outputPath, outputFile, dataEntry, flightTimeLimit, odorStim, randomCenter)
    % ---- SELECT RADIUS AND HEIGHT VALUES FOR THE VOLUME
    % Define  the size of the volume
    radius= 0.07;
    % Different heights for the volumes inside the Test Section
    volumeHeightValues= [0.04, 0.08, 0.16];
    h= volumeHeightValues(1);
    % -----------
    % Limit of the x axis in windTunnel
    lim_x= 0.9144;
    %Number of frames count inside the volume (base cue, tested cue and random volume in WT
    countsPerPosition= zeros(1,length(dataEntry.expCues(2:end,1))+1);
    ...
    for objID= unique(dataEntry.attr_id)' %uniqueID()
        ...
        %Load the frames where appears the current objID
        %objFrames= dataEntry.attr_id == objID; %return 0 or 1 per row regarding condition
        ...
        if duration >= flightTimeLimit
            ...
            % of times the insect objID has been near the cue
             for colorUsed = 2:length(dataEntry.expCues(:,1))+1                        
                % Load XY position for 1st visual clue
                if colorUsed > length(dataEntry.expCues(:,1))
                    x= randomCenter(1);
                    y= randomCenter(2);
                else
    ...
    if nnz(insectCtrInPos)
        ...
        [counts posX] = groupcounts(insectCtrInPos(:,1));
        countsPerPosition(posX)= counts;
        ...
        writetable(insectCtrTable, outputFile, 'Sheet', odorStim, 'Range', 'A1');
        countsPerPosition= [0, 0, 0];
    end
    %disp(unique(insectCtrInPos(:,1)))
    %disp(countsPerPosition)
    %disp(' --- ')
