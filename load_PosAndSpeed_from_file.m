...
%Returns: object ids, frame numbers and X,Y,Z velocities per frame

function [attr_id, attr_time, attr_frame, attr_x, attr_y, attr_z, attr_xvel, attr_yvel, attr_zvel]= load_posAndSpeed_from_file(file, loadFullDataset)
    ...
    if loadFullDataset == true
        ...
        attr_xvel=double(dataset.xvel);
        attr_yvel=double(dataset.yvel);
        attr_zvel=double(dataset.zvel);     
        ...
        attr_xvel=dataset.xvel(1:halfSize);
        attr_yvel=dataset.yvel(1:halfSize);
        attr_zvel=dataset.zvel(1:halfSize);     
